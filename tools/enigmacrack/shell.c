 ...
 *   show config              cfg   Show current Enigma + shell state
...
static void cmd_ldfreq(const char* path) {
    ...
    g_cfg.flags |= 2; /* enable frequency matching in scoring */
    g_freq_loaded = 1;
...
static void cmd_set(const char* args) {
    ...
    if (!strcmp(attr, "method") || !strcmp(attr, "meth") || !strcmp(attr, "m")) {
        ...
        if (!val || !*val) {
            printf("Usage: set plugboard <ABCD>  (letter pairs, e.g. 'ABCD' for A<->B, C<->D)\n");
            return;
        }
...
static void cmd_analyze(void) {
    ...
    /* ── Rotor identity ── */
    int r_on[4] = {
        !!(g_vary & SHELL_VARY_R0),
        !!(g_vary & SHELL_VARY_R1),
        !!(g_vary & SHELL_VARY_R2),
        !!(g_vary & SHELL_VARY_R3),
    };
    ...
    int rp_on[4] = {
        !!(g_vary & SHELL_VARY_R0P),
        !!(g_vary & SHELL_VARY_R1P),
        !!(g_vary & SHELL_VARY_R2P),
        !!(g_vary & SHELL_VARY_R3P),
    };
    ...
    /* ── Reflector ── */
    if (g_vary & SHELL_VARY_REFLECTOR) {
    ...
    /* ── Plugboard ── */
    if (g_vary & SHELL_VARY_PLUGBOARD) {
...
static ScoreFunc pick_score_func(void) {
    ...
    if (g_method == SHELL_METHOD_NGRAM) {
        if (!g_cfg.ngrams) {
